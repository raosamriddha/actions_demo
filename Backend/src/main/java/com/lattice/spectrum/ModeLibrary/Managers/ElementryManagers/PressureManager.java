...
public class PressureManager {
                    ...
                    if (valveProp[i] != null) {
                        if (valveProp[i].getTarget() != PressureTarget.PERMEATE) {
                            ...
                            if (recirculationSpeedFactor == 1) {
                                ...
                                } else if (currentPressureError < -0.2) {
    ...
    public void start(ValveProp[] valveProp, RecirculationProp recirculationProp, FlowProp flowProp, ModeListener superCallback, boolean secondValveInUse) {
        ...
        if (valveProp != null) {
            ...
            int i = 0;
            secondValve = valveProp.length==2;
            for (ValveProp v : valveProp) {
                if(v!=null) {
                    ...
                    if (v.isAutoMode()) {
                        ...
                        sLog.d(PressureManager.this, valveSetPos[i] + " Starting Pressure Control");
                    }
                }
                i++;
            }
    ...
    public void stop() {
        ...
        if (valveProp != null) {
            ...
            for (ValveProp v : valveProp) {
                if(v!=null) {
                    if (v.getTarget() == PressureTarget.PERMEATE) {
                        ComLib.get().getValveInfo().setValvePosition(i, (int) valveProp[i].getClosePosition(), 0);
                    } else {
                        ComLib.get().getValveInfo().setValvePosition(i, 1, 0);
                    }
                }
                i++;
            }
    ...
    public void stop(boolean secondValve) {
        DeltaPressureManager.get().stop(secondValve);
        pressureControlListener.stopRxListening();
        if(secondValve) {
        ...
        for (ValveProp v : valveProp) {
            if(v!=null) {
                if (v.isAutoMode()) {
                    ...
                    sLog.d(PressureManager.this, valveSetPos[i] + " Starting Pressure Control again");
                }
            }
            i++;
        }
